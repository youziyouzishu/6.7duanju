...
class UserController extends Base
{
    ...
    function bindWechat(Request $request)
    {
        ...
        $user = User::where(['platform_open_id' => $response->getId()])->first();
        if ($user) {
    ...
    function bindParent(Request $request)
    {
        $invitecode = $request->post('invitecode');
        $user = User::find($request->user_id);
        if (!$parent = User::where('invitecode', $invitecode)->where('id', '<>', $user->id)->first()) {
            return $this->fail('邀请码不存在');
        }
        if ($user->parent) {
            return $this->fail('您已绑定过上级');
        }
